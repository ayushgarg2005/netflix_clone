...
import React, { useState, useEffect } from "react";
import { Search, Bell, User, LogOut, Settings, HelpCircle, X, ChevronDown } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
...
const Navbar = () => {
  ...
  const profileMenuItems = [
    { icon: <User size={16} />, text: "Profile", path: "/profile" },
    { icon: <Settings size={16} />, text: "Account", path: "/account" },
    { icon: <HelpCircle size={16} />, text: "Help Center", path: "/help" }
  ];
        ...
        <div className="flex items-center gap-4 lg:gap-12">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span className={`absolute -bottom-1 left-0 h-[2px] bg-[#e50914] transition-all duration-300 
                ...
                <div className="bg-[#141414] border border-white/10 shadow-2xl rounded-lg overflow-hidden backdrop-blur-xl">
                  ...
                  <div className="p-4 border-b border-white/5 bg-white/5 flex items-center gap-3">
                     ...
                     <div>
                       ...
                       <p className="text-[10px] text-[#46d369] font-black uppercase tracking-tighter">Premium</p>
                     </div>
                  </div>
                  ...
                  <div className="p-2">
                    {profileMenuItems.map((item, i) => (
                      <button 
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 text-[13px] text-gray-300 hover:bg-white/10 hover:text-white rounded-md transition-all"
                      >
                        {item.icon} {item.text}
                      </button>
                    ))}
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-4 bg-white/5 text-white text-[11px] font-black uppercase tracking-widest hover:bg-[#e50914] transition-all duration-300"
                  >
                    <LogOut size={14} /> Sign Out of Streamflix
                  </button>
                </div>
