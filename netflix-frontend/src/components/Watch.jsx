...
import { Check, Settings, Video, Users, Plus, LogIn, X, Play } from "lucide-react"; // ✅ Added Icons
...
const Watch = () => {
    ...
    const initPlayer = async () => {
      try {
        const [movieRes, progressRes] = await Promise.all([
          ...
          axios.get(`http://localhost:5000/api/progress/${id}`, {
            withCredentials: true,
          }),
        ]);
  ...
  const handleCreateRoom = async () => {
    ...
    const adminId = await axios.get("http://localhost:5000/api/auth/me", {
      withCredentials: true,
    });
    const res = await axios.post("http://localhost:5000/api/room/add", {
      roomId: roomId,
      movieId: id,
      adminId: adminId.data.user.id,
    });
  ...
  const handleJoinRoom = async () => {
    const res = await axios.get(
      `http://localhost:5000/api/room/search/${joinRoomId}`
    );
    if (!res.data.found) {
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-gray-400 text-xs">
                        Start a new party and invite others
                      </p>
                    </div>
                  </div>
                </button>
